...
# Adapted from Python for Finance by Yves Hilpisch (O'Reilly). Copyright
# 2015 Yves Hilpisch, 978-1-491-94528-5.
...
for t in range(1, M + 1, 1):
    S[t] = S[t - 1] * (np.exp((r - rj - 0.5 * sigma**2) * dt + sigma *
                              np.sqrt(dt) * sn1[t]) + (np.exp(mu + delta * sn2[t]) - 1) * poi[t])
